...
```
The mean is **`r round(mean_steps_per_day,2)`** <br>
The median is **`r median_steps_per_day`**
...
The **`r idx_max_step_by_interval`** interval contains the maximum number(**`r  max_step_by_interval`**) of steps.
...
```{r, fig.height=8, warning=FALSE, message=FALSE, error=FALSE }
